...
canonical = {
             'channel': 'slutload', 
             'host': config.get_setting("current_host", 'slutload', default=''), 
             'host_alt': ["https://slutload.com/"], 
             'host_black_list': [], 
...
finds = {'find': {'find_all': [{'tag': ['div'], 'class': ['index-module__item--bqpMZ']}]},     #'id': re.compile(r"^browse_\d+")}]},
         'categories': {'find_all': [{'tag': ['li'], 'class': ['category']}]}, 
         'categories': dict([('find', [{'tag': ['div'], 'class': ['index-module__grid--Vk_RC']}]),
                             ('find_all', [{'tag': ['a']}])]),
         'search': {}, 
         'get_quality': {}, 
         'get_quality_rgx': '', 
         ...
         'last_page': dict([('find', [{'tag': ['div'], 'class': ['index-module__pagination--StbXY']}]), 
                            ('find_all', [{'tag': ['button'], '@POS': [-2], '@ARG': 'aria-label'}])]), 
         'plot': {}, 
         'findvideos': {},
...
def mainlist(item):
    ...
    itemlist.append(Item(channel=item.channel, title="Nuevos" , action="list_all", url=host + "porn?p=1"))
    itemlist.append(Item(channel=item.channel, title="Mas Vistos" , action="list_all", url=host + "porn/popular?p=1"))
    # itemlist.append(Item(channel=item.channel, title="Mejor valorado" , action="list_all", url=host + "exclusive-videos"))
    itemlist.append(Item(channel=item.channel, title="Pornstars" , action="section", url=host + "porn/pornstar?p=1", extra="PornStar"))
    ...
    for elem in matches_int:
        ...
        try:
            ...
            elem_json['thumbnail'] = elem.img.get('data-thumb_url', '') or elem.img.get('data-original', '') \
                                     ...
                                     or elem.img.get('src', '')
            datos = elem.span.text.split('-')
            elem_json['stime'] = datos[-1]
...
def search(item, texto, **AHkwargs):
    ...
    item.url = "%sporn/search/%s?p=1" % (host, texto.replace(" ", "+"))
