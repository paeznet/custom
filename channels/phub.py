...
canonical = {
             'channel': 'phub', 
             'host': config.get_setting("current_host", 'phub', default=''), 
             'host_alt': ["https://phub.at/"], 
             'host_black_list': [], 
...
finds = {'find': dict([('find', [{'tag': ['div'], 'class': ['main']}]),
         ...
         'profile_labels': {
                            'list_all_title': dict([('find', [{'tag': ['h2']}]),
                             ...
                             'list_all_premium': {'find': [{'tag': ['img'], 'src': re.compile(r"xtapes")}]}     #NETU
                            },
...
def play(item):
    ...
    if "xtapes" in item.thumbnail:
        url = soup.find('a', href=re.compile("https://xtapes.to"))['href']       # NETU
    
    if soup.find('div', class_='entry').find(re.compile("(?:iframe|source)")):
        url = soup.find('div', class_='entry').find(re.compile("(?:iframe|source)"))['src']
