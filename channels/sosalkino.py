...
canonical = {
             ...
             'host_alt': ["https://wvw.sosalkino.tube/"], 
             'host_black_list': [], 
...
finds = {'find': dict([('find', [{'tag': ['div'], 'class': ['items-list', 'video_list']}]),
                       ('find_all', [{'tag': ['div'], 'class': ['item', 'video-item']}])]),     #'id': re.compile(r"^browse_\d+")}]},
         ...
         'url_replace': [], 
         'profile_labels': {
                            # 'list_all_stime': {'find': [{'tag': ['span'], 'class': ['text'], '@TEXT': '(\d+:\d+)' }]},
...
def mainlist(item):
    ...
    itemlist.append(Item(channel=item.channel, title="Nuevos" , action="list_all", url=host + "1/"))
    ...
    itemlist.append(Item(channel=item.channel, title="Favoritos" , action="list_all", url=host + "most-popular/month/"))
...
def list_all(item):
    ...
    if item.extra == 'Search':
        findS['find'] = dict([('find', [{'tag': ['div'], 'id': ['list_videos_videos_list_search_result']}]),
...
def search(item, texto, **AHkwargs):
    ...
    item.url = "%ssearch/?q=%s&sort_by=video_viewed&from_videos=1" % (host, texto.replace(" ", "+"))
