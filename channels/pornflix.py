...
canonical = {
             'channel': 'pornflix', 
             'host': config.get_setting("current_host", 'pornflix', default=''), 
             'host_alt': ["https://pornoflix.xxx/"], 
             'host_black_list': [], 
...
def section(item):
    ...
    findS = finds.copy()
    findS['url_replace'] = [['(\/(?:category|channels|models|pornstars|actor)\/[^$]+$)', r'\1page/1/?filter=latest']]
...
def play(item):
    ...
    if soup.find('div', id='video-actors'):
        pornstars = soup.find('div', id='video-actors').find_all('a', href=re.compile(r"/actor/[a-z0-9-]+"))
    ...
    if soup.find('div', class_='responsive-player').find(re.compile("(?:iframe|source)")):
        ...
        if "php?q=" in url:
            url_decode = AlfaChannel.convert_url_base64(url, host)
