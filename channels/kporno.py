...
canonical = {
             'channel': 'kporno', 
             'host': config.get_setting("current_host", 'kporno', default=''), 
             'host_alt': ["https://kporno.com/"], 
             'host_black_list': [], 
...
def mainlist(item):
    ...
    itemlist.append(Item(channel=item.channel, title="PornStar" , action="categorias", url=host + "camgirls/"))
...
def categorias(item):
    ...
    for elem in matches:
        url = elem.a['href'].replace("?actors=", "/actor/")
        title = elem.a['title']
