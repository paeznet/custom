...
canonical = {
             'channel': 'pornslash', 
             'host': config.get_setting("current_host", 'pornslash', default=''), 
             'host_alt': ["https://www.pornslash.com/"], 
             'host_black_list': [], 
...
finds = {'find': {'find_all': [{'tag': ['div'], 'class': ['video-item']}]},     #'id': re.compile(r"^browse_\d+")}]},
         'categories': {'find_all': [{'tag': ['a', 'div'], 'class': ['cat-item', 'model-item']}]}, 
         'search': {}, 
         'get_quality': {}, 
         'get_quality_rgx': '', 
         'next_page': dict([('find', [{'tag': ['div'], 'class': ['pager']}]),
...
def mainlist(item):
    ...
    itemlist.append(Item(channel=item.channel, title="Nuevos" , action="list_all", url=host + "videos/new?p=1"))
    itemlist.append(Item(channel=item.channel, title="Mejor valorado" , action="list_all", url=host + "videos/?p=1"))
    itemlist.append(Item(channel=item.channel, title="Mas Vistos " , action="list_all", url=host + "search/all?p=1&sort=view"))
    itemlist.append(Item(channel=item.channel, title="Trending" , action="list_all", url=host + "search/all?p=1"))
    itemlist.append(Item(channel=item.channel, title="Novedades" , action="list_all", url=host + "search/all?p=1&sort=date"))
    itemlist.append(Item(channel=item.channel, title="Mas largo" , action="list_all", url=host + "search/all?p=1&sort=duration"))
...
def section(item):
    logger.info()
    # cat/big-tits?sort=date
    findS = finds.copy()
    findS['url_replace'] = [['(\/(?:cat|pornstar)\/[^$]+$)', r'\1?sort=date']]
...
def play(item):
    ...
    if soup.find_all('a', class_="video-model"):
        pornstars = soup.find_all('a', class_="video-model")
