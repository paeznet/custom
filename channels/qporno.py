...
canonical = {
             'channel': 'qporno', 
             'host': config.get_setting("current_host", 'qporno', default=''), 
             'host_alt': ["https://qporno.xxx/"], 
             'host_black_list': [], 
...
finds = {'find': {'find_all': [{'tag': ['section'], 'class': ['pst-cn']}]},     #'id': re.compile(r"^browse_\d+")}]},
         'categories': {'find_all': [{'tag': ['section'], 'class': ['pst-cn']}]}, 
         'search': {}, 
         'get_quality': {}, 
         'get_quality_rgx': '', 
         'next_page': {},
...
def mainlist(item):
    ...
    itemlist.append(Item(channel=item.channel, title="Canal" , action="section", url=host + "webs/", extra="Canal"))
...
def play(item):
    ...
    url = soup.find('div', class_='vdcn').iframe['src']
