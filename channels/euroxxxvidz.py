...
canonical = {
             'channel': 'euroxxxvidz', 
             'host': config.get_setting("current_host", 'euroxxxvidz', default=''), 
             'host_alt': ["https://euroxxxvidz.com/"], 
             'host_black_list': [], 
...
finds = {'find': dict([('find', [{'tag': ['div'], 'class': ['posts-wrapper']}]),
                       ('find_all', [{'tag': ['article'], 'class': re.compile(r"^post-\d+")}])]),
         'categories': dict([('find', [{'tag': ['div'], 'class': ['entry-content']}]),
                             ('find_all', [{'tag': ['a'], 'href': re.compile("/category/")}])]),
         'search': {}, 
         'get_quality': {}, 
         'get_quality_rgx': '', 
         'next_page': {},
         'next_page_rgx': [['\/page\/\d+\/', '/page/%s/']], 
         'last_page': dict([('find', [{'tag': ['ul'], 'class': ['page-numbers']}]), 
...
def mainlist(item):
    ...
    itemlist.append(Item(channel = item.channel, title="Nuevos" , action="list_all", url=host + "page/1/?s"))
