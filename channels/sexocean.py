...
canonical = {
             'channel': 'sexocean', 
             'host': config.get_setting("current_host", 'sexocean', default=''), 
             'host_alt': ["https://sexocean.net/"], 
             'host_black_list': [], 
...
finds = {'find': {'find_all': [{'tag': ['div'], 'class': ['list__item']}]},     #'id': re.compile(r"^browse_\d+")}]},
         'categories': dict([('find', [{'tag': ['li'], 'class': ['js-categories-item']}]),
                             ('find_all', [{'tag': ['a'], 'href': re.compile(r"^/category/\d+")}])]),
         'search': {}, 
         'get_quality': {}, 
         'get_quality_rgx': '', 
         'next_page': {},
         ...
         'url_replace': [], 
         'profile_labels': {
                            'list_all_stime': dict([('find', [{'tag': ['time']}]),
...
def mainlist(item):
    ...
    itemlist.append(Item(channel=item.channel, title="Nuevos" , action="list_all", url=host + "p/1" ))
    itemlist.append(Item(channel=item.channel, title="Top" , action="list_all", url=host + "top/month/1" ))
