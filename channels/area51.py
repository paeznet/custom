...
canonical = {
             'channel': 'area51', 
             'host': config.get_setting("current_host", 'area51', default=''), 
             'host_alt': ["https://area51.porn/"], 
             'host_black_list': [], 
...
def mainlist(item):
    ...
    itemlist.append(Item(channel=item.channel, title="Canal" , action="categorias", url=host + "channels/?sort_by=rating&from=01"))
...
def lista(item):
    ...
    for elem in matches:
        ...
        if not thumbnail.startswith("https"):
            thumbnail = "https:%s" % thumbnail
        time = elem.find('span', class_='is-hd')
        if "HD" in time.text:
