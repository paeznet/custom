...
canonical = {
             'channel': 'heroero', 
             'host': config.get_setting("current_host", 'heroero', default=''), 
             'host_alt': ["https://heroero.com/"], 
             'host_black_list': [], 
...
def mainlist(item):
    ...
    itemlist.append(Item(channel=item.channel, title="PornStar" , action="categorias", url=host + "actress/?sort_by=total_videos&from=01"))
...
def search(item, texto):
    logger.info()
    texto = texto.replace(" ", "-")
    item.url = "%ssee/%s/?sort_by=post_date&from_videos=01" % (host,texto)
    try:
        return lista(item)
    except:
        import sys
...
def lista(item):
    ...
    for elem in matches:
        ...
        if not thumbnail.startswith("https"):
            thumbnail = "https:%s" % thumbnail
        time = elem.find('span', class_='item-panel-col').text.strip()
        quality = elem.find('span', class_='col-hd')
        if quality:
