...
def mainlist(item):
    ...
    itemlist.append(Item(channel=item.channel, title="Mas vistos" , action="lista", url=host + "?sort_by=video_viewed&from=01"))
    itemlist.append(Item(channel=item.channel, title="Mejor valorado" , action="lista", url=host + "?sort_by=rating&from=01"))
    ...
    itemlist.append(Item(channel=item.channel, title="PornStar" , action="categorias", url=host + "pornstars?sort_by=avg_videos_popularity&from=01"))
    itemlist.append(Item(channel=item.channel, title="Canal" , action="categorias", url=host + "porn-review?sort_by=total_videos&from=01"))
...
def categorias(item):
    ...
    for elem in matches:
        ...
        if "porn-review" in item.url:
            cantidad = elem.find_all('div', class_='videos-title')[1]
            cantidad = cantidad.text.strip()
            cantidad = cantidad.split()[0]
        else:
            cantidad = elem.find('i', class_='icon-blockplay')
            if cantidad:
                cantidad = cantidad.parent.text.strip()
        if cantidad:
...
def lista(item):
    ...
    matches = matches[-1]
    for elem in matches.find_all('div', class_='item'):
        url = elem.a['href']
        title = elem.a['title']
        thumbnail = elem.img['src']
        if "gif" in thumbnail:
        ...
        if not thumbnail.startswith("https"):
            thumbnail = "https:%s" % thumbnail
        pornstars = elem.find('div', class_='model-name')
        if pornstars:
            pornstars = pornstars.find_all('a')
            for x , value in enumerate(pornstars):
                pornstars[x] = value.text.strip()
            pornstar = ' & '.join(pornstars)
            pornstar = "[COLOR cyan]%s[/COLOR]" % pornstar
        time = elem.find_all('span', class_='item__flag--duration')
