...
def mainlist(item):
    logger.info()
    itemlist = []
    itemlist.append(Item(channel=item.channel, title="Nuevos" , action="lista", url=host + "?sort_by=post_date&from=01"))
    ...
    itemlist.append(Item(channel=item.channel, title="PornStar" , action="categorias", url=host + "pornosztarok/?sort_by=total_videos&from=01"))
    itemlist.append(Item(channel=item.channel, title="Canal" , action="categorias", url=host + "szexcsatorna/?sort_by=total_videos&from=01"))
    itemlist.append(Item(channel=item.channel, title="Categorias" , action="categorias", url=host + "szex-kategoriak/?sort_by=title"))
...
def search(item, texto):
    logger.info()
    texto = texto.replace(" ", "-")
    item.url = "%skereses/?q=%s&sort_by=post_date&from_videos=01" % (host,texto)
    try:
        return lista(item)
    except:
        import sys
...
def play(item):
    ...
    itemlist = []
    soup = create_soup(item.url)
    pornstars = soup.find('div', class_='info').find_all('a', href=re.compile("/pornosztarok/[A-z0-9-]+/"))
