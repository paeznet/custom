...
canonical = {
             'channel': 'xxxiporn', 
             'host': config.get_setting("current_host", 'xxxiporn', default=''), 
             'host_alt': ["https://xxxi.porn/"], 
             'host_black_list': ['https://rusoska.com/'], 
             'pattern': ['url" content="([^"]+)"'], 
...
def mainlist(item):
    ...
    itemlist.append(Item(channel=item.channel, title="PornStar" , action="categorias", url=host + "porno-models?sort_by=total_videos&from=01"))
    itemlist.append(Item(channel=item.channel, title="Canal" , action="categorias", url=host + "sites?sort_by=total_videos&from=01"))
    itemlist.append(Item(channel=item.channel, title="Categorias" , action="categorias", url=host + "porno-cats?sort_by=title"))
...
def play(item):
    ...
    for x , value in enumerate(pornstars):
        pornstars[x] = value.span.text.strip()
