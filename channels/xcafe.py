...
canonical = {
             'channel': 'xcafe', 
             'host': config.get_setting("current_host", 'xcafe', default=''), 
             'host_alt': ["https://xcafe.com/"], 
             'host_black_list': [], 
...
def categorias(item):
    ...
    matches = soup.find_all('a', class_='popfire')
    for elem in matches:
        url = elem['href']
        title = elem.img['alt']
    ...
    if next_page:
        next_page = next_page.find('span', class_='current')
...
def lista(item):
    ...
    itemlist = []
    soup = create_soup(item.url)
    matches = soup.find_all('li', attrs={"data-video_id": re.compile(r"\d+")})
    for elem in matches:
        url = elem.a['href']
        title = elem.img['alt']
        thumbnail = elem.img['src']
