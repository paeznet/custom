...
canonical = {
             'channel': 'faapy', 
             'host': config.get_setting("current_host", 'faapy', default=''), 
             'host_alt': ["https://faapy.com/"], 
             'host_black_list': [], 
...
def mainlist(item):
    ...
    itemlist.append(Item(channel=item.channel, title="Mas vistos" , action="lista", url=host + "most-popular/?sort_by=video_viewed_month&from=02"))
    itemlist.append(Item(channel=item.channel, title="Mejor valorado" , action="lista", url=host + "top-rated/?sort_by=rating_month&from=02"))
...
def categorias(item):
    ...
    for elem in matches:
        ...
        cantidad = elem.find('div', class_='column')
        if cantidad:
...
def play(item):
    ...
    itemlist = []
    soup = create_soup(item.url)
    pornstars = soup.find_all('a', href=re.compile("/model/[A-z0-9-]+/"))
