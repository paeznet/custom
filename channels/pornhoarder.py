...
def list_all(item):
    ...
    for elem in matches:
        ...
        if elem.find('img'):
           ...
           title = "[COLOR orange]%s[/COLOR] %s" % (canal,title)
...
def play(item):
    ...
    url = videos(item.url)
    if soup.find('ul', class_='server-list') and "netu" in item.title or not url:
        server2 = soup.find('ul', class_='server-list').find_all('a')
        url = server2[0]['href']
        url = urlparse.urljoin(host,url)
        url = videos(url)
