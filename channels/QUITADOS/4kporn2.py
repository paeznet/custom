...
https://4kporn.xxx/latest-updates/?mode=async&function=get_block&block_id=list_videos_latest_videos_list&sort_by=post_date&from=2
https://4kporn.xxx/search/?q=big+tits&sort_by=post_date&from_videos=1
...
def lista(item):
    ...
    itemlist = []
    soup = create_soup(item.url)
    logger.debug(soup.find('div', class_='list-videos'))
