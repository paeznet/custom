...
def findvideos(item):
    ...
    itemlist = []
    soup = create_soup(item.url)
    if soup.find('meta', itemprop='embedURL'):
        url = soup.find('meta', itemprop='embedURL')['content']
    else:
        url = soup.find('meta', itemprop='contentURL')['content']
    if "mediadelivery" in url:
        url = soup.iframe['src']
        soup = create_soup(url,host)
        url = soup.find('source', type='application/x-mpegURL')['src']
        url += "|Referer=https://iframe.mediadelivery.net/"
