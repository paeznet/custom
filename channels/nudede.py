...
canonical = {
             'channel': 'nudede', 
             'host': config.get_setting("current_host", 'nudede', default=''), 
             'host_alt': ["https://www.nudede.com/"], 
             'host_black_list': [], 
...
finds = {'find': dict([('find', [{'tag': ['div', 'main'], 'id': ['primary', 'main']}]),
         ...
         'plot': {}, 
         'findvideos': {},
         'title_clean': [['[\(|\[]\s*[\)|\]]', ''],['(?i)\s*videos*\s*', ''],['(?i)\s*View all post filed under *\s*', '']],
...
def mainlist(item):
    ...
    itemlist.append(Item(channel=item.channel, title="Pornstars" , action="section", url=host + "celebrities/page/1/", extra="PornStar"))
    itemlist.append(Item(channel=item.channel, title="Pais" , action="section", url=host + "tags/page/1/", extra="Canal")) 
...
def play(item):
    ...
    #####   mediadelivery
    # https://www.nudede.com/jennifer-lawrence-desnuda-en-no-hard-feelings/
    # url = "https://iframe.mediadelivery.net/embed/6214/8821d470-7ee6-4faf-b871-52d2efbc76e0?autoplay=false&loop=false&muted=false&preload=false"
    if "mediadelivery" in url:
        # url = soup.iframe['src']
        mediadelivery = url
        from core import httptools
        data = httptools.downloadpage(url, headers={'Referer': host}).data
        # logger.debug(data)
        url = scrapertools.find_single_match(data, 'loadSource\("([^"]+)"')
        url += "|Referer=%s" %mediadelivery
        itemlist.append(['[mediadelivery] .mp4', url])
    else:
