...
canonical = {
             'channel': 'xxk', 
             'host': config.get_setting("current_host", 'xxk', default=''), 
             'host_alt': ["https://xxk.mobi/"], 
             'host_black_list': [], 
...
finds = {'find': {'find_all': [{'tag': ['div'], 'class': ['card']}]},
...
def mainlist(item):
    ...
    itemlist.append(Item(channel=item.channel, title="Nuevos" , action="list_all", url=host + "1.html"))
    itemlist.append(Item(channel=item.channel, title="Popular" , action="list_all", url=host + "popular/1.html"))
...
def search(item, texto, **AHkwargs):
    ...
    item.url = "%sfree/%s/" % (host, texto.replace(" ", "-"))
