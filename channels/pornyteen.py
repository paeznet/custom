...
canonical = {
             'channel': 'pornyteen', 
             'host': config.get_setting("current_host", 'pornyteen', default=''), 
             'host_alt': ["https://pornyteen.com/"], 
             'host_black_list': [], 
...
def mainlist(item):
    ...
    itemlist.append(Item(channel=item.channel, title="Canal" , action="categorias", url=host + "paysites/"))
...
def categorias(item):
    ...
    matches = soup.find('section', class_='-sec-main').find_all('div', class_=re.compile(r"^(?:c|m)item__inner"))
    for elem in matches:
        url = elem.a['href']
        title = elem.a['title']
...
def lista(item):
    ...
    itemlist = []
    soup = create_soup(item.url)
    matches = soup.find_all('div', class_='item__inner')
    for elem in matches:
        url = elem.a['href']
        title = elem.a['title']
        thumbnail = elem.img['src']
