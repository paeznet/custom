...
canonical = {
             'channel': 'sxyland', 
             'host': config.get_setting("current_host", 'sxyland', default=''), 
             'host_alt': ["https://sxyland.com/"], 
             'host_black_list': [], 
...
def section(item):
    ...
    findS['url_replace'] = [['(\/(?:categories|category|channels|models|pornstars|actor)\/[^$]+$)', r'\1page/1/?filter=latest']]
