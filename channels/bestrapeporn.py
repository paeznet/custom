...
canonical = {
             'channel': 'bestrapeporn', 
             'host': config.get_setting("current_host", 'bestrapeporn', default=''), 
             'host_alt': ["https://bestrapeporn.com/"], 
             'host_black_list': [], 
...
finds = {'find': dict([('find', [{'tag': ['main'], 'id': ['main']}]),  ##[{'tag': ['div'], 'class': ['videos-list']}]),
...
def section(item):
    ...
    findS = finds.copy()
    findS['url_replace'] = [['(\/(?:category|actor)\/[^$]+$)', r'\1?page/1/?filter=latest']]
