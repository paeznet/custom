...
def mainlist(item):
    ...
    itemlist.append(Item(channel=item.channel, title="Canal" , action="categorias", url=host + "categories/?sort_by=title"))
...
def catalogo(item):
    logger.info()
    itemlist = []
    soup = create_soup(item.url).find('div', class_='list-tags')
