...
canonical = {
             'channel': 'watch_my_gf', 
             'host': config.get_setting("current_host", 'watch_my_gf', default=''), 
             'host_alt': ["https://watch-my-gf.com/"], 
             'host_black_list': [], 
...
finds = {'find':  dict([('find', [{'tag': ['div'], 'class': ['video-list', 'list-videos', 'thumbs_video', 'thumbs_list', 'thumbs', 'items-videos']}]),
         ...
         'categories': dict([('find', [{'tag': ['div'], 'class': ['models_list', 'list-models', 'category_list', 'list-categories', 'video-list', 'list-videos', 'list-channels', 'list-sponsors', 'thumbs', 'thumbs_list', 'categories_list']}]),
                             ('find_all', [{'tag': ['div', 'a', 'span'], 'class': ['item', 'video-item', 'thumb', 'holder-item', 'th', 'model_items']}])]),
         'search': {}, 
         'get_quality': {}, 
         'get_quality_rgx': '', 
...
def mainlist(item):
    logger.info()
    itemlist = []
    itemlist.append(Item(channel=item.channel, title="Nuevas" , action="list_all", url=host + "results?sort_by=post_date&from_videos=1"))
    itemlist.append(Item(channel=item.channel, title="Mas Vistas" , action="list_all", url=host + "results?sort_by=video_viewed_month&from_videos=1"))
    itemlist.append(Item(channel=item.channel, title="Mejor Valorada" , action="list_all", url=host + "results?sort_by=rating_month&from_videos=1"))
    itemlist.append(Item(channel=item.channel, title="Mas Favoritas" , action="list_all", url=host + "results?sort_by=most_favourited&from_videos=1"))
    itemlist.append(Item(channel=item.channel, title="Mas Comentadas" , action="list_all", url=host + "results?sort_by=most_commented&from_videos=1"))
    itemlist.append(Item(channel=item.channel, title="Mas Largas" , action="list_all", url=host + "results?sort_by=duration&from_videos=1"))
...
def play(item):
    ...
    if soup.find_all('span', class_='model_items'):
        pornstars = soup.find_all('span', class_='model_items')
...
def search(item, texto, **AHkwargs):
    ...
    item.url = "%sresults?query=%s&sort_by=post_date&from_videos=1" % (host, texto.replace(" ", "+"))
