...
canonical = {
             'channel': 'sex3', 
             'host': config.get_setting("current_host", 'sex3', default=''), 
             'host_alt': ["https://sex3.com/"], 
             'host_black_list': [], 
...
def mainlist(item):
    ...
    itemlist.append(Item(channel=item.channel, title="PornStar" , action="categorias", url=host + "stars/?sort_by=rating"))
...
def canal(item):
    ...
    matches = soup.find_all('a', href=re.compile(r"^/channel/[a-z0-9-]+/"))
    for elem in matches:
...
def categorias(item):
    ...
    itemlist = []
    soup = create_soup(item.url)
    matches = soup.find('ul', class_='thumbs').find_all('li')
    for elem in matches:
        url = elem.a['href']
        title = elem.img['alt']
        if elem.find('span', class_='no-thumb'):
...
def lista(item):
    ...
    itemlist = []
    soup = create_soup(item.url)
    matches = soup.find_all('div', class_='fuck')
    for elem in matches:
        url = elem.a['href']
        title = elem.a['title']
        thumbnail = elem.img['src']
