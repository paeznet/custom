...
canonical = {
             'channel': 'saintporn', 
             'host': config.get_setting("current_host", 'saintporn', default=''), 
             'host_alt': ["https://saintporn.com/"], 
             'host_black_list': [], 
             'pattern': ['base href="([^"]+)"'], 
...
def mainlist(item):
    ...
    itemlist.append(Item(channel = item.channel, title="Canal", action="section", url=host + "sites/?sort_by=title&from=1", extra="Canal"))
