...
canonical = {
             'channel': 'hornybutt', 
             'host': config.get_setting("current_host", 'hornybutt', default=''), 
             'host_alt': ["https://www.hornybutt.com/"], 
             'host_black_list': [], 
...
def categorias(item):
    ...
    for elem in matches:
        ...
        url = elem.a['href']
        title = elem.a['title']
        if elem.img and elem.img.get("data-src", ""):
...
def lista(item):
    ...
    for elem in matches:
        thumbnail = ""
        url = elem.a['href']
        title = elem.a['title'].replace("<strong>", "").replace("</strong>", "")
        if elem.img:
            thumbnail = elem.img['data-src']
