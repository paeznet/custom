...
def lista(item):
    ...
    for elem in matches:
        url = elem['href']
        title = elem.find(class_='title-video2').text.strip()
    ...
    if next_page:
        next_page=next_page.find_all('a')[-1]
        text = soup.find('div', class_='pagination').text.strip()
        number = [int(temp)for temp in text.split() if temp.isdigit()]
    if next_page and number[0] != number[1]:
