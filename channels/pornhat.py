...
#   https://www.pornhat.com  https://ok.xxx https://ok.porn
canonical = {
             'channel': 'pornhat', 
             'host': config.get_setting("current_host", 'pornhat', default=''), 
             'host_alt': ["https://www.pornhat.com/"], 
             'host_black_list': [], 
...
def mainlist(item):
    ...
    itemlist.append(Item(channel=item.channel, title="Categoria" , action="categorias", url=host + "mix/"))
...
def categorias(item):
    ...
    for elem in matches:
        if not elem.find('div', class_='thumb-ctr'):
            continue
        url = elem.a['href']
        title = elem.a['title']
