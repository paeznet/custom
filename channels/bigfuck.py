...
def mainlist(item):
    ...
    itemlist.append(Item(channel=item.channel, title="PornStar" , action="categorias", url=host + "stars/"))
...
def categorias(item):
    ...
    if "category/" in item.url:
        soup = soup.find('div', id='galleries')
    matches = soup.find_all('div', class_='b-thumb-item__inner')
    for elem in matches:
        url = elem.a['href']
        title = elem.img['alt']
        ...
        if cantidad:
            title = "%s (%s)" % (title,cantidad.text.strip().replace(" Vids", ""))
...
def lista(item):
    ...
    itemlist = []
    soup = create_soup(item.url)
    matches = soup.find('div', id='galleries').find_all('div', class_='b-thumb-item__inner')
    for elem in matches:
        url = elem.a['href']
        title = elem.img['alt']
        thumbnail = elem.img['src']
