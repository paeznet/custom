...
canonical = {
             'channel': 'bigtitslust', 
             'host': config.get_setting("current_host", 'bigtitslust', default=''), 
             'host_alt': ["https://www.bigtitslust.com/"], 
             'host_black_list': [], 
...
def lista(item):
    ...
    itemlist = []
    soup = create_soup(item.url)
    matches = soup.find_all('div', class_='list-videos')[-1]
    matches = matches.find_all('div', class_='item')
    for elem in matches:
        if elem.find('span', class_='is-av'):
            continue
        url = elem.a['href']
        title = elem.a['title']
        thumbnail = elem.img['src']
