...
canonical = {
             'channel': 'alphaporno', 
             'host': config.get_setting("current_host", 'alphaporno', default=''), 
             'host_alt': ["https://www.alphaporno.com/"], 
             'host_black_list': [], 
...
def mainlist(item):
    ...
    itemlist.append(Item(channel=item.channel, title="Mas largo" , action="lista", url=host + "longest/1/"))
    itemlist.append(Item(channel=item.channel, title="PornStar" , action="categorias", url=host + "pornstars/top-rated/1/"))
...
def categorias(item):
    ...
    if "pornstars/" in item.url:
        matches = soup.find('ul', class_='models-list').find_all('li')
    else:
    ...
    for elem in matches:
        ...
        if "pornstars/" in item.url:
            ...
            title = elem.img['alt']
            cantidad = elem.find('span', class_='cat-title')
    ...
    next_page = soup.find('li', class_='btn-next')
    if next_page:
