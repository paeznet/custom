...
canonical = {
             ...
             'host_alt': ["https://www.alluretube.com/"], 
             'host_black_list': [], 
...
# https://www.alluretube.com/video/1834/oriental-blue-1975-classic-porn-movie
# <script>
# let vid_files = '{"vid_files":['+
                # '{"src":"cDdxMmp5SWZGL0pwNWZVSVQrZFNNQzBYeW0rVFFtRTlnVEtXQTRUZEZLRnczL0ZJNjNOTXJob01xV0xnZUJsUVpBVUk2L1Y2R1JrL3ZwOWYxSTBZbFE9PQ==","type":"video/mp4", "label":"1080p", "res":"1080" },'+ 
                # '{"src":"cDdxMmp5SWZGL0pwNWZVSVQrZFNNQzBYeW0rVFFtRTlnVEtXQTRUZEZLRnczL0ZJNjNOTXJob01xV0xnZUJsUU1aTDgxLzBJa2JrQ2NDaWx4aUdZb2c9PQ==","type":"video/mp4", "label":"480p", "res":"480" },'+ 
# </script>

# https://www.alluretube.com/media/videos/h264/1834_480p.mp4
# https://www.alluretube.com/media/videos/h264/1834_1080p.mp4
...
def play(item):
    ...
    soup = soup.find('div', class_='content-left').script.string
    matches = scrapertools.find_multiple_matches(soup, '"label":"([^"]+)"')
    id = scrapertools.find_single_match(item.url, '/video/(\d+)/')
    for quality in matches:
        url = "%smedia/videos/h264/%s_%s.mp4" %(host, id, quality)
        itemlist.append(['[alluretube] %s' %quality, url])
