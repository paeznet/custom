...
canonical = {
             'channel': 'czmyporno', 
             'host': config.get_setting("current_host", 'czmyporno', default=''), 
             'host_alt': ["https://myporno.cz/"], 
             'host_black_list': [], 
...
finds = {'find':  dict([('find', [{'tag': ['div'], 'class': ['video-list']}]),
         ...
         'last_page': dict([('find', [{'tag': ['div'], 'class': ['paginator']}]), 
...
def play(item):
    ...
    # if soup.find('div', id='kt_player'):
        # url = item.url
    # else:
    url = soup.find('div', class_='main-video').iframe['src']
