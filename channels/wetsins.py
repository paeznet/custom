...
canonical = {
             'channel': 'wetsins', 
             'host': config.get_setting("current_host", 'wetsins', default=''), 
             'host_alt': ["https://www.wetsins.com/"], 
             'host_black_list': [], 
...
def categorias(item):
    ...
    matches = soup.main.find_all('div', class_=re.compile(r"-(?:model|channel)"))
    for elem in matches:
        url = elem.a['href']
        title = elem.a['title']
...
def lista(item):
    ...
    for elem in matches:
        ...
        if not thumbnail.startswith("https"):
            thumbnail = "https:%s" % thumbnail
        time = elem.find('span', class_='item-time').text.strip()
        quality = elem.find('span', class_='item-quality')
        if quality:
