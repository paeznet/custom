...
canonical = {
             'channel': 'goodporn', 
             'host': config.get_setting("current_host", 'goodporn', default=''), 
             'host_alt': ["https://goodporn.to/"], 
             'host_black_list': [], 
...
def mainlist(item):
    ...
    itemlist.append(Item(channel=item.channel, title="PornStar" , action="categorias", url=host + "models/?sort_by=avg_videos_rating&from=01"))
    itemlist.append(Item(channel=item.channel, title="Canal" , action="categorias", url=host + "channels/?sort_by=avg_videos_popularity&from=01"))
    itemlist.append(Item(channel=item.channel, title="Sitios" , action="categorias", url=host + "sites/?sort_by=avg_videos_popularity&from=01"))
...
def categorias(item):
    ...
    matches = soup.find('div', class_='box').find_all('a')
    for elem in matches:
        url = elem['href']
        title = elem['title']
        thumbnail = elem.img
