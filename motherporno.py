...
# https://www.motherporno.com   https://www.babestube.com/
canonical = {
             'channel': 'motherporno', 
             'host': config.get_setting("current_host", 'motherporno', default=''), 
             'host_alt': ["https://www.motherporno.com"], 
             'host_black_list': [], 
...
def mainlist(item):
    ...
    itemlist.append(Item(channel=item.channel, title="Mas largo" , action="lista", url=host + "/longest/?is_hd=&sort_by=duration&from=01"))
...
def search(item, texto):
    logger.info()
    texto = texto.replace(" ", "-")
    item.url = "%s/search/%s/?is_hd=&sort_by=post_date&from_videos=01" % (host,texto)
    try:
        return lista(item)
    except:
        import sys
...
def lista(item):
    ...
    next_page = soup.find_all('li', class_='item pager')
    if next_page:
        next_page = next_page[-1].a['data-parameters'].split(":")[-1]
