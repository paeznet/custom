...
canonical = {
             'channel': 'thepornarea', 
             'host': config.get_setting("current_host", 'thepornarea', default=''), 
             'host_alt': ["https://thepornarea.com"], 
             'host_black_list': [], 
...
def search(item, texto):
    logger.info()
    texto = texto.replace(" ", "-")
    item.url = "%s/search/%s/?sort_by=post_date&from=01" % (host,texto)
    try:
        return lista(item)
    except:
        import sys
...
def categorias(item):
    ...
    matches = soup.find_all('div', class_='th item')
    for elem in matches:
        url = elem.a['href']
        title = elem.img['alt']
        thumbnail = elem.img['data-original']
        cantidad = elem.find('div', class_='ttime')
        if cantidad:
...
def lista(item):
    ...
    for elem in matches:
        ...
        quality = elem.find('div', class_='hd')
        if quality:
