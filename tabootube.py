...
canonical = {
             'channel': 'tabootube', 
             'host': config.get_setting("current_host", 'tabootube', default=''), 
             'host_alt': ["https://www.tabootube.xxx"], 
             'host_black_list': [], 
...
def mainlist(item):
    ...
    itemlist.append(Item(channel=item.channel, title="PornStar" , action="catalogo", url=host + "/models/?sort_by=total_videos&from=01"))
    itemlist.append(Item(channel=item.channel, title="Canal" , action="catalogo", url=host + "/sites/?sort_by=total_videos&from=01"))
...
def categorias(item):
    ...
    matches = soup.find('div', class_='navigation').find_all('a', href=re.compile("/categories/"))
    for elem in matches:
        url = elem['href']
