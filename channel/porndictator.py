...
# https://www.porndictator.com    https://www.babestube.com    https://www.momvids.com     https://www.pornomovies.com  
# https://www.deviants.com
canonical = {
             'channel': 'porndictator', 
             'host': config.get_setting("current_host", 'porndictator', default=''), 
             'host_alt': ["https://www.porndictator.com"], 
             'host_black_list': [], 
...
def categorias(item):
    ...
    for elem in matches:
        ...
        cantidad = elem.find_all('span', class_='col')
        if cantidad:
            title = "%s (%s)" % (title,cantidad[1].text.strip())
        url = url.replace("https://www.babestube.com", "")
